    ...
    if line.p0.z == line.p1.z and line.p1.z == plane:
        ...
        if t >= 0 and t <= 1:
            ...
            if testZ <= max(line.p0.z, line.p1.z) and testZ >= min(line.p0.z, line.p1.z):
                ...
                return testP  ##Point(x_=line.p0.x+t*slope.x, y_=line.p0.y+t*slope.y, z_=line.p0.z+t*slope.z)
...
def writeGcode(slices,filename):
    ...
    for s in slices:
        ...
        for l in s.perimeter:
            ...
            f.write("G0 X"+str(o+l.p0.x)+" Y"+str(o+l.p0.y)+" Z"+str(l.p0.z)+ " F2700\n")
            #move to end while extruding
            dist = math.sqrt(pow(l.p1.x-l.p0.x,2) + pow(l.p1.y-l.p0.y,2))
            E += dist*extrudeRate
            f.write("G1 X"+str(o+l.p1.x)+" Y"+str(o+l.p1.y)+" E"+str(E)+" F900\n")
        ...
        for l in s.support:
            ...
            dist = math.sqrt(pow(l.p1.x-l.p0.x,2) + pow(l.p1.y-l.p0.y,2))
            E += dist*extrudeRate
            f.write("G1 X"+str(o+l.p1.x)+" Y"+str(o+l.p1.y)+" E"+str(E)+" F800\n")
...
def slicing(fn, lt, infill):
    filename = fn # sys.argv[1]
    layerThickness = lt # float(sys.argv[2])
    infillPercent = infill # float(sys.argv[3])
